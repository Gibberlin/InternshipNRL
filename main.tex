...
\titleformat{\chapter}[block]
  {\normalfont\large\bfseries\color{blue}\centering}
...
\begin{center}
    \vspace*{1cm}
    {\Large \textbf{\color{blue}CERTIFICATE}}
\end{center}
...
Name: \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_ \hfill
Name: \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_ \\

Designation: \_\_\_\_\_\_\_\_\_\_\_\_\_ \hfill
...
\begin{center}
    \vspace*{1cm}
    {\Large \textbf{\color{blue}DECLARATION}}
\end{center}
...
\begin{center}
    \vspace*{1cm}
    {\Large \textbf{\color{blue}ACKNOWLEDGEMENT}}
\end{center}
...
\begin{center}
    \vspace*{1cm}
    {\Large \textbf{\color{blue}ABSTRACT}}
\end{center}
...
\begin{tikzpicture}[>=stealth, 
  stepblock/.style={rectangle, draw, fill=blue!5, text width=2.4cm, align=center, rounded corners, minimum height=1.2cm, font=\small}]
  
  \node[stepblock] (ingest) at (0,0) {Ingestion\\(\texttt{log\_collector.py})};
  \node[stepblock] (norm) at (3.3,0) {Normalization\\(\texttt{ingress.py})};
  \node[stepblock] (correlate) at (6.6,0) {Correlation\\(\texttt{engine.py})};
  \node[stepblock] (orchestrate) at (9.9,0) {Orchestration\\(\texttt{siem\_server.py})};
  \node[stepblock] (contain) at (13.2,0) {Containment\\(\texttt{block\_ip} Playbook)};
  
  \draw[->, thick] (ingest) -- (norm);
  \draw[->, thick] (norm) -- (correlate);
  \draw[->, thick] (correlate) -- (orchestrate);
  \draw[->, thick] (orchestrate) -- (contain);
  \draw[->, thick, dashed] (contain) .. controls (6.6, -1.8) .. (ingest);
\end{tikzpicture}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{images/system-architecture-flow.png}
...
The platform was evaluated using controlled attack simulations against the \texttt{vulnerable\_app.py} honeypot, while telemetry was shipped through \texttt{agent.py}, \texttt{log\_collector.py}, and the ingress gateway. The measured results emphasise why a refinery cannot rely on manual human response for deterministic threats.
...
\footnotesize
\begin{tabularx}{\textwidth}{|p{2.5cm}|X|X|p{1.5cm}|}
\hline
...
\footnotesize
\begin{tabularx}{\textwidth}{|X|p{4cm}|p{3cm}|p{3.5cm}|}
\hline
...
\begin{tcolorbox}[colback=blue!5,colframe=blue!65!black,title=Engineering Trade-off Summary]
...
\begin{itemize}
  \item Packaging independent execution environments using custom Dockerfiles (\texttt{Dockerfile.siem}, \texttt{Dockerfile.agent}, \texttt{Dockerfile.collector}, \texttt{Dockerfile.vulnerable}).
...
Annexure I — Core Configuration: OT Digital Twin simulation sandbox
...
Annexure II — Logstash Pipeline Configuration: Logstash event processing pipeline
...
Annexure III — Filebeat Collector Manifest: Filebeat security agent configuration
...
Sample JSON request body transmitted from client agents (EDR host agent) to the central Ingress API endpoint (/api/ingress):
